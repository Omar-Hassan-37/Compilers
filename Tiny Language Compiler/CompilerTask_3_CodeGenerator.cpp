...
// added for, from ,to ,inc, startfor, endfor, break token types
enum TokenType
{
    FOR,
    FROM,
...
// added for, from ,to ,inc, startfor, endfor, break token types string
const char *TokenTypeStr[] =
    {
        "For", "From", "To", "Inc", "StartFor", "EndFor", "Break", "If", "Then", "Else", "End", "Repeat", "Until", "Read", "Write",
...
// added for, from ,to ,inc, startfor, endfor, break token types to reserved words
...
// modified the ebnf to accept forstmt by adding line 312

// program -> stmtseq
// stmtseq -> stmt { ; stmt }
// stmt -> forstmt | ifstmt | repeatstmt | assignstmt | readstmt | writestmt | breakstmt
//*****************************************************************************************
...
// mathexpr -> absdiff { (+|-) absdiff }    left associative
// absdiff -> term { & term} lefr associative
...
const char *NodeKindStr[] =
    {
        "For", "If", "Repeat", "Assign", "Read", "Write",
        "Oper", "Num", "ID", "Break"};
...
//forstmt -> for identifier from mathexpr to mathexpr inc mathexpr startfor stmtseq endfor
//added forstmt function to handle the for stmt parsing
TreeNode *ForStmt(CompilerInfo *pci, ParseInfo *ppi)
{
    ...
    Match(pci, ppi, FOR); // match for
    if (ppi->next_token.type == ID)
        AllocateAndCopy(&tree->id, ppi->next_token.str);
    Match(pci, ppi, ID); // matchs the identifier that comes after for
    Match(pci, ppi, FROM);
    tree->child[0] = MathExpr(pci, ppi); //matches from then go to mathexpr to match the number that comes after from
    Match(pci, ppi, TO);
    tree->child[1] = MathExpr(pci, ppi); //matches to then go to mathexpr to match the number that comes after to
    Match(pci, ppi, INC);                // matches inc

    // added an if condition to handle negative input after inc
    if (ppi->next_token.type == MINUS)
    {
        // added new_tree to handle the operation minus
        TreeNode *new_tree = new TreeNode;
        new_tree->node_kind = OPER_NODE;
        new_tree->oper = ppi->next_token.type;
        new_tree->line_num = pci->in_file.cur_line_num;

        // added negative tree to put zero before the minus operation as (0 - x)
        TreeNode *negativeTree = new TreeNode;
        negativeTree->node_kind = NUM_NODE;
        negativeTree->num = 0;

        // merged the two trees together
        new_tree->child[0] = negativeTree;
        Match(pci, ppi, ppi->next_token.type);
        new_tree->child[1] = Term(pci, ppi); // go to term to match the number that comes after minus

        // added the new_tree to tree (parent tree)
        tree->child[2] = new_tree;
    }
    // else the inc number is not negative
    else
    {
        tree->child[2] = MathExpr(pci, ppi); // go to mathexpr to match the number that comes after  inc
    }
    Match(pci, ppi, STARTFOR);
    tree->child[3] = StmtSeq(pci, ppi); // matchs startfor then goes to stmtseq to match the stmt in the for loop
    Match(pci, ppi, ENDFOR);            // matchs end for

    pci->debug_file.Out("End ForStmt"); // end for stmt
    return tree;
}
...
TreeNode *Stmt(CompilerInfo *pci, ParseInfo *ppi)
{
    ...
    if (ppi->next_token.type == FOR)
        tree = ForStmt(pci, ppi);
    else if (ppi->next_token.type == IF)
        tree = IfStmt(pci, ppi);
    ...
    else if (ppi->next_token.type == BREAK)
        tree = BreakStmt(pci, ppi);
    else
        throw 0;
...
TreeNode *StmtSeq(CompilerInfo *pci, ParseInfo *ppi)
{
    ...
    // added end for condition to the while loop
    while (ppi->next_token.type != ENDFILE && ppi->next_token.type != END && ppi->next_token.type != ENDFOR &&
...
void Analyze(TreeNode *node, SymbolTable *symbol_table)
{
    ...
    // added error conditions for handeleing non integer inputs in the for loop
    if (node->node_kind == FOR_NODE && node->child[0]->expr_data_type != INTEGER)
        printf("ERROR from works only on INTEGER\n");
    if (node->node_kind == FOR_NODE && node->child[1]->expr_data_type != INTEGER)
        printf("ERROR to works only on INTEGER\n");
    if (node->node_kind == FOR_NODE && node->child[2]->expr_data_type != INTEGER)
        printf("ERROR inc works only on INTEGER\n");
...
{
    //added if condition for for node
    if (node->node_kind == FOR_NODE && !node->loopBreak || node->child[0]->node_kind == FOR_NODE)
    {
        if (node->child[2]->node_kind == OPER_NODE)
        {
            int start = Evaluate(node->child[0], symbol_table, variables);
            variables[symbol_table->Find(node->id)->memloc] = start;
            int end = Evaluate(node->child[1], symbol_table, variables);
            int count = Evaluate(node->child[2], symbol_table, variables);
            while (start > end)
            {
                RunProgram(node->child[3], symbol_table, variables);
                if (node->child[3]->loopBreak)
                {
                    //node->loopBreak = true;
                    break;
                }
                variables[symbol_table->Find(node->id)->memloc] = start + count;
